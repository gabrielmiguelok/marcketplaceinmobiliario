...
interface QuestionData {
  ...
  minLabel?: string; // Para slider
  maxLabel?: string; // Para slider
}
...
const QUESTIONS: QuestionData[] = [
  {
    ...
    type: 'single-select',
    options: ["Tranquilo y residencial 🌳", "Dinámico y urbano 🏙️", "Mixto: cerca de todo pero sin tanto ruido 🏢🌿"]
  },
  {
    id: 2,
  ...
  },
  {
    ...
    type: 'multi-select',
    options: ["Gimnasio 💪", "Áreas verdes 🌿", "Piscina 🏊", "Seguridad 24/7 🔒", "Coworking 💻", "Pet-friendly 🐶", "Kids zone 👶"]
  },
  {
    id: 4,
    question: "¿Cuál es tu rango de presupuesto aproximado?",
    type: 'single-select',
    options: ["Q500,000 – Q800,000", "Q800,000 – Q1,200,000", "Más de Q1,200,000"]
  },
  {
    id: 5,
    question: "¿Qué tan importante es para ti la seguridad de la zona?",
    type: 'single-select',
    options: ["Muy importante 🔒", "Moderadamente importante 😐", "No es mi prioridad 🚶"]
  },
  {
    id: 6,
    question: "¿Qué tan importante es vivir cerca de tu trabajo, colegios o zonas comerciales?",
    type: 'slider',
...
const NavLink = ({ text, isBold = false, href = "#" }: { text: string, isBold?: boolean, href?: string }) => (
  <a href={href} className={`text-[#0B1B32] hover:text-[#00F0D0] transition-colors duration-200 text-[15px] ${isBold ? 'font-bold' : 'font-medium'}`}>
    {text}
  </a>
);

const UserBadge = ({ initials }: { initials: string }) => (
      ...
      {Array.from({ length: totalSteps }).map((_, idx) => {
        ...
        return (
          <React.Fragment key={idx}>
             {/* Círculo */}
            <div className={`w-3 h-3 rounded-full transition-colors duration-300 ${isActive ? 'bg-[#00F0D0]' : 'bg-gray-200'}`} />
            {/* Línea conectora (excepto el último) */}
            {idx < totalSteps - 1 && (
              <div className={`w-8 h-1 transition-colors duration-300 ${stepNum < currentStep ? 'bg-[#00F0D0]' : 'bg-gray-200'}`} />
            )}
          </React.Fragment>
        );
      })}
...
export default function App() {
  const [currentStep, setCurrentStep] = useState(0); // 0 = Hero, 1-6 = Quiz, 7 = Results
    ...
    setSelectedOptions(prev => {
      ...
      if (isMulti) {
        ...
        if (arrayCurrent.includes(option)) {
          return { ...prev, [stepId]: arrayCurrent.filter(item => item !== option) };
        } else {
  ...
  const renderHero = () => (
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in zoom-in duration-500">
      <h1 className="text-4xl md:text-[56px] leading-[1.1] font-bold text-[#0B1B32] tracking-tight">
        Queremos ayudarte a encontrar<br className="hidden md:block" />
  ...
  // 2. Render de las Preguntas (Slide)
    ...
    return (
      <div className="w-full max-w-2xl mx-auto animate-in fade-in slide-in-from-right-8 duration-300">
        ...
        <div className="mb-10 space-y-3">
          {questionData.type === 'single-select' || questionData.type === 'multi-select' ? (
            questionData.options?.map((option, idx) => (
              <button
                key={idx}
                onClick={() => toggleOption(questionData.id, option, questionData.type === 'multi-select')}
              ...
              >
                <span>{option}</span>
                {isSelected(questionData.id, option) && <Check size={20} className="text-[#00F0D0]" />}
              </button>
            ))
          ) : questionData.type === 'slider' ? (
            <div className="py-10 px-4">
               {/* Estilos CSS inline para el slider personalizado para asegurar compatibilidad cross-browser sin plugins externos */}
              <style>{`
                input[type=range] {
              ...
              <input 
                ...
                style={{
                  // Esto crea el efecto de "relleno" progresivo en el track
  ...
  // 3. Render de Resultados Finales
  const renderResults = () => (
    <div className="w-full max-w-[1200px] mx-auto animate-in fade-in zoom-in duration-500 pb-10">
      <div className="text-center mb-10">
        ...
        <p className="text-lg text-[#0B1B32] font-medium max-w-2xl mx-auto">
          Basado en tus respuestas, encontramos proyectos que encajan con tu estilo de vida y presupuesto.
        </p>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-6 mb-12 px-4">
        ...
        <div className="bg-[#0B1B32] text-white p-8 rounded-[2rem] relative">
          ...
          <p className="text-lg leading-relaxed opacity-90">
            Zona 15: segura, residencial y con buena accesibilidad.
            Ideal si buscas tranquilidad sin alejarte del centro.
          </p>
          {/* Triángulo decorativo estilo 'speech bubble' invertido */}
          <div className="absolute -bottom-4 left-10 w-8 h-8 bg-[#0B1B32] transform rotate-45 hidden md:block"></div> 
        </div>

        {/* Card 2 */}
        <div className="bg-[#0B1B32] text-white p-8 rounded-[2rem] relative">
          <h3 className="text-2xl font-bold mb-4">Conclusion 2</h3>
          <p className="text-lg leading-relaxed opacity-90">
            Zona 15: segura, residencial y con buena accesibilidad.
            Ideal si buscas tranquilidad sin alejarte del centro.
          </p>
        ...
        {/* Card 3 */}
        <div className="bg-[#0B1B32] text-white p-8 rounded-[2rem] relative">
          <h3 className="text-2xl font-bold mb-4">Conclusion 3</h3>
          <p className="text-lg leading-relaxed opacity-90">
            Zona 15: segura, residencial y con buena accesibilidad.
            Ideal si buscas tranquilidad sin alejarte del centro.
          </p>
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-[#0B1B32] flex flex-col relative overflow-hidden">
      
      {/* --- NAVBAR (Fijo) --- */}
      <nav className="w-full max-w-[1400px] mx-auto px-6 py-6 flex items-center justify-between relative z-50">
        <div className="flex-shrink-0 cursor-pointer" onClick={() => setCurrentStep(0)}>
          <span className="text-4xl font-semibold tracking-tight leading-none flex items-center gap-1">
            <span className="relative inline-block">
              a
              <div className="absolute top-[6px] left-[3px] w-[6px] h-[6px] bg-white rounded-full"></div>
            </span>
            loba
          </span>
        </div>
        ...
        <div className="hidden md:flex items-center space-x-6 font-bold text-sm">
          ...
          <UserBadge initials="SK" />
        </div>

        <div className="md:hidden">
          <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="text-[#0B1B32] focus:outline-none p-2">
      ...
      {/* --- CONTENIDO PRINCIPAL --- */}
      <main className="flex-grow flex flex-col items-center justify-center text-center px-4 md:px-6 py-8">
