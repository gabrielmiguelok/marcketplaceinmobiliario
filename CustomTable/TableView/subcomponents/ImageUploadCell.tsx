...
export default function ImageUploadCell({
  ...
  const handleClick = useCallback((e: React.MouseEvent) => {
    ...
    if (!canUpload || isClickingRef.current) {
      return
    }
  ...
  const getImageSrc = () => {
    ...
    if (imageUrl) {
      if (imageUrl.startsWith('/productos/') || imageUrl.startsWith('/comprobantes/')) {
        return `/api/imagen${imageUrl}?t=${cacheKey}`
      }
  ...
  return (
    <div
      ...
      title={canUpload ? "Click para cambiar imagen" : ""}
      onMouseEnter={(e) => {
        if (canUpload) {
          e.currentTarget.style.borderColor = '#3b82f6'
