/**
 * Archivo: /components/CustomTable/toolbar/components/ToggleFilterButtons.tsx
 */
'use client';
...
// Hasta 4 estados
function getColorFromState(state: number) {
  switch (state) {
    case 1:
      return '#00ff0a'; // verde
    case 2:
...
export interface ToggleFilterButtonsProps {
  whatsAppState: number;                       // 0|1|2
  setWhatsAppState: (fn: (prev: number) => number) => void;

  correoActive: boolean;                       // false|true
  setCorreoActive: (fn: (prev: boolean) => boolean) => void;

  destacadoState: number;                      // 0|1|2
  setDestacadoState: (fn: (prev: number) => number) => void;

  googleMyBusinessState: number;               // 0|1|2
  setGoogleMyBusinessState: (fn: (prev: number) => number) => void;

  webState: number;                            // 0|1|2|3
  setWebState: (fn: (prev: number) => number) => void;
}
...
function ToggleFilterButtons({
  ...
  // 3 estados genérico
  const cycleGeneric3States = (current: number) => {
    if (current === 0) return 1;
    if (current === 1) return 2;
    return 0;
  };
  // GMB => 0->2->1->0
  const cycleGMB3States = (current: number) => {
    if (current === 0) return 2;
    if (current === 2) return 1;
    return 0;
  };
  // Web (4 estados) => 0->1->2->3->0
  const cycleWeb4States = (current: number) => {
    if (current === 0) return 1;
    if (current === 1) return 2;
