...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const precioFormateado = inmueble.moneda === 'USD'
    ? `$${(Number(inmueble.precio_usd) || 0).toLocaleString('en-US')}`
    : `Q${(Number(inmueble.precio_gtq) || 0).toLocaleString('es-GT')}`
  ...
  const canonicalUrl = `${baseUrl}/proyectos/${inmueble.id}/${slug}`
  ...
  return {
    ...
    other: {
      'og:price:amount': inmueble.precio_usd.toString(),
      'og:price:currency': inmueble.moneda,
      'product:price:amount': inmueble.precio_usd.toString(),
...
export default async function InmuebleDetailPage({ params }: Props) {
  ...
  if (urlSlug !== correctSlug) {
    redirect(`/proyectos/${inmueble.id}/${correctSlug}`)
  }
  ...
  const canonicalUrl = `${baseUrl}/proyectos/${inmueble.id}/${correctSlug}`
              ...
              <div className="bg-white rounded-3xl p-6 shadow-lg border border-gray-100 sticky top-6">
                <div className="text-center mb-6 pb-6 border-b border-gray-100">
                  ...
                  <div className="text-3xl md:text-4xl font-bold text-[#00F0D0]">
                    <PriceDisplay
                      precioUsd={inmueble.precio_usd}
                      precioGtq={inmueble.precio_gtq}
                    />
                  </div>
                </div>
