...
export default function UserMapaPage() {
  ...
  const [userInfo, setUserInfo] = useState<UserInmobiliaria | null>(null)
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`/api/inmuebles/by-user?usuario=${userInmobiliaria}`)
  ...
  const fetchFiltersWithParams = useCallback(async (isInitial = false) => {
    ...
    try {
      ...
      params.set('usuario', userInmobiliaria)
  ...
  }, [userInmobiliaria, zonaFilter, tipoFilter, operacionFilter, precioMaxFilter])
  ...
  const displayName = userInfo?.full_name || userInfo?.first_name || userInmobiliaria
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">
                {inmuebles.length} propiedades en mapa
              </p>
            </div>
          </div>
